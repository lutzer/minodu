...
from minodu_forum.src.celery.tasks import convert_file_task, ConversionResult
from minodu_forum.src.celery.file_converter import convert_audio, convert_image
...
def test_convert_image_file():
    ...
    try:
        ...
        result = convert_file_task(0, temp_file, converted_file)
...
def test_convert_audio_file(input):
    ...
    try:
        ...
        result = convert_file_task(0, temp_file, converted_file, input_type=input[1])
...
def test_convert_file_callback(httpserver: HTTPServer):
    ...
    try:
        ...
        result = convert_file_task(result.file_id, result.input_file, result.output_file, callback_url=httpserver.url_for("/api/callback"))
