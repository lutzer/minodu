...
from .auth import generate_token, get_author_from_token
...
async def create_author(author: AuthorCreate, db: Session = Depends(get_db)):
    ...
    try:
        # create author
        db_author = Author(name=author.name, avatar_id=author.avatar).validate()
    ...
    return AuthorCreateResponse(id=db_author.id, token=token)
